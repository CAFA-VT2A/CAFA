    ...
    def forward(self, item):
        ...
        if self.do_offset:
            ...
            if vid.shape[0] < v_end_i:
                # print(is_random_crop)
                print(v_start_i)
                v_end_i = vid.shape[0]
                v_crop_len_frames = v_end_i - v_start_i
                
                self.crop_len_sec = frames2sec(v_crop_len_frames, v_fps)
        ...
        if aud.shape[0] < a_end_i:
            print(f"audio start: {a_start_i}")
            a_end_i = aud.shape[0]
            #Roi: i think the following line is a mistake: but it ddont vchagne something anyway Imremoveing it::
            # v_crop_len_frames = a_end_i - a_start_i
        ...
        assert item['video'].shape[0] == round(v_fps * self.crop_len_sec), f'{item["video"].shape} {item["path"]} and {v_fps * self.crop_len_sec}'
        assert item['audio'].shape[0] == round(a_fps * self.crop_len_sec), f'{item["audio"].shape} {item["path"]} and {a_fps * self.crop_len_sec}'
...
class GenerateMultipleSegments(torch.nn.Module):
    ...
    def get_sequential_seg_ranges(self, v_len_frames, a_len_frames, v_fps, a_fps, n_seg, seg_size_aframes):
        ...
        if self.is_start_random:
            ...
            #Roi: we want our embedding to start from the begging of the vodeo.
            v_start_i = 0
...
if __name__ == '__main__':
    ...
    # fn = GenerateMultipleSegments(segment_size_vframes=16, n_segments=34,
    #                             is_start_random=False, audio_jitter_sec=0.05, step_size_seg=0.5)
    
    fn = GenerateMultipleSegments(segment_size_vframes=16, n_segments=34,
                            is_start_random=False, audio_jitter_sec=0.05, step_size_seg=0.5625)
    input = fn(input)
    print(input['audio'].shape, input['video'].shape, input['meta']['audio'])
