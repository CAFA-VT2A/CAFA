...
def copy_state_dict(model, state_dict):
    ...
    # Track matches of each type
    direct_matches = 0
    transformer_matches = 0
    pretransform_matches = 0
    unmatched = 0
    ...
    # Second pass - try alternate keys for unmatched params
    for key in model_state_dict:
        if key in state_dict:
            ...
            if (
                modified_key in state_dict
                and state_dict[modified_key].shape == model_state_dict[key].shape
            ):
                ...
                transformer_matches += 1
            else:
                # Try pretransform match
                modified_key = key.replace(
                    "model.model.controlnet.prepare_conditioned_audio",
                    "pretransform.model",
                )
                if (
                    modified_key in state_dict
                    ...
                    if isinstance(state_dict[modified_key], torch.nn.Parameter):
                        state_dict[modified_key] = state_dict[modified_key].data
                    model_state_dict[key] = state_dict[modified_key]
                    pretransform_matches += 1
                else:
                    # print(f"Unmatched parameter: {key}")
                    unmatched += 1

    # Print final stats
    print(f"\nParameter matching summary:")
    print(f"Direct matches: {direct_matches}")
    print(f"Transformer matches: {transformer_matches}")
    print(f"Pretransform matches: {pretransform_matches}")
    print(f"Unmatched parameters: {unmatched}")
    print(
        f"Total matches: {direct_matches + transformer_matches + pretransform_matches}"
    )

    model.load_state_dict(model_state_dict, strict=False)


## In case i want to load model which I trained:
def copy_state_dict_regular(model, state_dict):
    """Load state_dict to model, but only for keys that match exactly.
    ...
    for key in state_dict:
        if (
            ...
            changed_key = key.replace(
                "diffusion.", "", 1
            )  # Replace only the first occurrence
            if (
                changed_key in model_state_dict
                and state_dict[key].shape == model_state_dict[changed_key].shape
            ):
                if isinstance(state_dict[key], torch.nn.Parameter):
                    # backwards compatibility for serialized parameters
                    state_dict[key] = state_dict[key].data
                model_state_dict[changed_key] = state_dict[key]
            else:
                print(key)
